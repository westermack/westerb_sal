...
(function() {
    ...
    if(/(public\/|thesalonproject.com\/)(?!admin)(index.php)?/i.test(cur_url)) westerb_sal.cur_pg = 'public_index';
    else if(/admin\/(#)?(index.php(#)?)?$/i.test(cur_url)) westerb_sal.cur_pg = 'admin_index';
    else if(/admin\/list_albums.php/i.test(cur_url)) westerb_sal.cur_pg = 'list_albums';
    else if(/admin\/list_photos.php/i.test(cur_url)) westerb_sal.cur_pg = 'list_photos';
    else if(/admin\/create_album.php/i.test(cur_url)) westerb_sal.cur_pg = 'create_album';
    else if(/admin\/photo_upload.php/i.test(cur_url)) westerb_sal.cur_pg = 'photo_upload';
    else if(/admin\/login.php/i.test(cur_url)) westerb_sal.cur_pg = 'login';
    else if(/admin\/add_user.php/i.test(cur_url)) westerb_sal.cur_pg = 'add_user';
    else if(/admin\/list_users.php/i.test(cur_url)) westerb_sal.cur_pg = 'list_users';
    else if(/admin\/logfile.php/i.test(cur_url)) westerb_sal.cur_pg = 'logfile';
})();
...
{ writable: false, enumerable: true, configurable: true,
value: function() {
    if($('#blinker').size() === 1) $('#blinker').get(0).scrollIntoView();
    else if($('#login_prompt_wrapper') === 1) $('#login_prompt_wrapper').get(0).scrollIntoView();
}
});
...
'readjust_gallery_elems', { // Define Object.prototype.westerb_sal.readjust_gallery_elems
    value: function() {//console.log('readjust_gallery_elems() runs');
        ...
        if(westerb_sal.on_display_img_natural_width < 340) on_display_img.css('width', 340); //if the image's natural width is already below the '#outer_wrapper' div's 340 min-width, increase it to 340px and maintain it there
        else {
            on_display_img.css('width', adjusted_on_display_img_width);
            if(parseInt(on_display_img.css('width'),10) < 340) on_display_img.css('width', 340); //if, while scaling, the image's calculated width drops below the '#outer_wrapper' div's 340 min-width, increase it and maintain it at 340px
        }
        ...
        if(centering_div_width < 1140 && u_info_elems.eq(0).css('max-height') != '430px') u_info_elems.css('max-height', '-=65'); //prevents the '#unit_info' div from overlapping with the '#gallery_thumbs_wrapper' div as the window scales
        else if(centering_div_width > 1140 && u_info_elems.eq(0).css('max-height') != '495px') u_info_elems.css('max-height', '+=65');
        ...
        if($('#u_count').text() != '') {
            if(p_info_elem_h < 335 && on_display_img_h < 355 && parseInt(on_display_img.css('height')) > 370 && centering_div_width < 710)
                ...
                else centering_div.css('min-height', (48/*'#title' height*/+10/*account for image's border width*/+on_display_img_h+100/*account for the '.for_margin_div'*/+30/*the "padding" between the image and the thumbs wrapper*/)+'px'); //only the image's height is now the determining parameter
            }
...
if(typeof FormData !== 'undefined') {
    ...
    value: function(xhr) {
        ...
        for(var name in westerb_sal.form.fields) {
            ...
            if(name === 'submit') continue; //irrelevant here
...
value: function(param_name) {
    ...
    param_value_pairs.forEach(function(e,i,a) {
        ...
        if(this_param_value_pair[0] === param_name) param_val =  this_param_value_pair[1];
    })
...
{ writable: false, enumerable: true, configurable: true,
value: function(msg) {
    if(!msg) msg = ''; //if no specific message is provided, only the ellipsis will be shown and animated
    var elem = $(this); //wrap in jQuery for more convenient API
    elem.html(msg);
    westerb_sal.p_timers[0] = window.setTimeout(function() { elem.append("<span id=\"b_dot\" class=\"processing_ellipsis\">&bull;</span>") }, 200);
    westerb_sal.p_timers[1] = window.setTimeout(function() { elem.append("<span id=\"r_dot\" class=\"processing_ellipsis\">&bull;</span>") }, 420);
    westerb_sal.p_timers[2] = window.setTimeout(function() { elem.append("<span id=\"y_dot\" class=\"processing_ellipsis\">&bull;</span>") }, 670);
    westerb_sal.p_timers[3] = window.setTimeout(function() { elem.append("<span id=\"g_dot\" class=\"processing_ellipsis\">&bull;</span>") }, 920);
    westerb_sal.p_timers[4] = window.setTimeout(function() { elem.get(0).processing_msg(msg) }, 1120); //animate by calling recursively
}
});
...
if(!sessionStorage.getItem('public_index')) westerb_sal.store_page('public_index', Object.create(null));
if(!sessionStorage.getItem('list_albums')) westerb_sal.store_page('list_albums', Object.create(null));
if(!sessionStorage.getItem('list_photos')) westerb_sal.store_page('list_photos', Object.create(null));
...
value: function(pg_name) {
    if(!pg_name) pg_name = westerb_sal.cur_pg;
...
{ writable: false, enumerable: true, configurable: true,
value: function(s_name) {
    if(!s_name) s_name = westerb_sal.cur_pg+'_'+westerb_sal.pagination.set_group+'_'+'s'+westerb_sal.pagination.current_set;
...
{ writable: false, enumerable: true, configurable: true,
value: function(p_name) {
    if(!p_name) p_name = window.location.hash;
...
value: function(msg, type, duration, modal) { //REVISIT THE modal ARGUMENT IDEA
    ...
    if(!msg) msg = 'Something went wrong. Refresh the page and try again.';
    if(!type) type = 'error';
    if(!duration) duration = 4000;
    var slide_progress = false, slide_progress_elem = $('#upload_progress_percent');
    if((westerb_sal.cur_pg === 'photo_upload' || window.location.hash === '#photo_upload') && westerb_sal.progress_events) slide_progress = true;
    if(slide_progress) {
        var i = 0;
        function slide_progress_down() {
            ...
            if(parseInt(slide_progress_elem.css('top'),10) < 135) window.setTimeout(slide_progress_down,0);
        }
        
        function slide_progress_up() {
            slide_progress_elem.css('top',(parseInt(slide_progress_elem.css('top'),10)-5)+'px');
            if(parseInt(slide_progress_elem.css('top'),10) > 85) window.setTimeout(slide_progress_up,0);
        }
    }
    ...
    if(slide_progress) slide_progress_down();
    $('div.message_wrapper').slideDown(300);
    westerb_sal.msg_remove_timer = window.setTimeout(function() { if(slide_progress) slide_progress_up(); $('div.message_wrapper').slideUp(300, function() { $(this).remove() }); }, duration+300);
    westerb_sal.request_done = true;
    $('#wait_msg').fadeOut(10, function() { $(this).remove(); });
}
});
...
value: function(msg, type) {
    ...
    if(!msg) msg = 'Something went wrong while performing your request. Please refresh the page and try again';
    if(!type) type = 'error'
    ...
    function close_box() {
        box.parent('div').fadeOut(200, function() { $(this).remove(); });
...
{ writable: false, enumerable: true, configurable: true,
    value: function(message, type, duration) {
        if(!message) message = 'Something went wrong. Refresh the page and try again.';
        if(!type) type = 'error';
        ...
        $('#modal_msg').hide().text(message).fadeIn(200, function() { westerb_sal.msg_remove_timer = window.setTimeout(function() { $('#modal_msg').fadeOut(200, function() { $(this).remove(); }) }, duration); });
        westerb_sal.request_done = true;
...
value: function(e, modal_dialog) { //if the second argument is passed as "true", the function was invoked from a modal dialog and the element where the message is to be displayed needs to be changed accordingly
    ...
    if(modal_dialog) {
        ...
        window.setTimeout(function() { $('#modal_msg').fadeOut(200, function() { $(this).remove(); }) }, 2400);
    }
    
    else {
        if(e.type == 'submit') westerb_sal.display_msg('Please wait a moment...', 'notice', 2400);
        else {
            $('#wait_msg').remove();
            ...
            window.setTimeout(function() { $('#wait_msg').fadeOut(200, function() { $(this).remove(); }) }, 1900);
        }
    }
}
});
...
Object.defineProperties(westerb_sal.Scroll_Pagination.prototype, {
    ...
    calc_unit_count : {
        value: function() {
            if(westerb_sal.cur_pg === 'list_photos') return parseInt($('.clicked').attr('u_count'),10);
            else if(westerb_sal.cur_pg === 'list_albums') return parseInt($('.clicked_o_name').attr('u_count'),10);
            else if(westerb_sal.cur_pg === 'public_index') return parseInt($('#c_count').children('span').text(),10);
        }, writable: true, enumerable: true, configurable: true
    },
    ...
    value: function() {
        ...
        $('#outer_wrapper').find('#unit_wrapper img, #loaded_info').fadeOut(300, function() {
            if($(this).attr('id') === 'loaded_info') {
                ...
                if(parseInt($('#c_count').children('span').text()) > 8) $('#loaded_info').scroll(westerb_sal.load_more_public_comments);
            }
            else {
...
value: function(e) {
    ...
    else if(westerb_sal.blocking_dialog) westerb_sal.scroll_to_dialog();
    else if(!westerb_sal.request_done) westerb_sal.please_wait_msg(e);
    //else if((westerb_sal.cur_pg === 'list_photos' || window.location.hash === '#list_photos') && !westerb_sal.request_done) westerb_sal.please_wait_msg(e);
        ...
        try {
            ...
            if(!pre_stored_set) {
                ...
                if(westerb_sal.cur_pg === 'list_photos') {
                    westerb_sal.get_units_animation_timer = window.setTimeout(function() { $('table tbody').children().css('visibility','hidden'); },90);
...
value: function(e) {
    ...
    //else if((westerb_sal.cur_pg === 'list_photos' || window.location.hash === '#list_photos') && !westerb_sal.request_done) westerb_sal.please_wait_msg(e);
        ...
        try {
            ...
            else westerb_sal.pagination.current_set++;
            var pre_stored_set = westerb_sal.retrieve_set(westerb_sal.cur_pg+'_'+westerb_sal.pagination.set_group+'_'+'s'+westerb_sal.pagination.current_set);
            if(!pre_stored_set) {
                westerb_sal.request_done = false;
                if(westerb_sal.cur_pg === 'list_photos') {
...
{ writable: false, enumerable: true, configurable: true,
    value: function(e) {
        ...
        (function remove_elem() {
            ...
            if(cur_elem.hasClass('faded')) cur_elem.fadeOut((cur_elem.hasClass('field_msg'))?1:fade_duration, function() {
                $(this).remove();
                elem_count-- //decrement count to animate the next element, if there is one
                if(elem_count !== 0) setTimeout(remove_elem, stall_duration); //there is/are more elements to animate; call the function again in stall_duration ms
                    ...
                    else $('#main').trigger('elems_remaval_complete'); //trigger the event that lets '#main' know that it's now time to remove the itself and replace with the new '#main' div content
                }
            });
            else if(cur_elem.hasClass('slided')) cur_elem.hide((cur_elem.attr('id')==='nav' || cur_elem.attr('id')==='photos_wrapper')?200:hide_duration, function() {
                $(this).remove();
                elem_count-- //decrement count to animate the next element, if there is one
                if(elem_count !== 0) setTimeout(remove_elem, stall_duration); //there is/are more elements to animate; call the function again in stall_duration ms
                else if(elem_count === 0) { //there are no more elements of the class to animate
...
{ writable: false, enumerable: true, configurable: true,
    value: function() {
        ...
        $('div.message_wrapper').slideUp(10, function() { $(this).remove(); });
        $('#wait_msg').fadeOut(10, function() { $(this).remove(); });
...
'handle_hash_change', { // Define Object.prototype.westerb_sal.handle hash change
    value: function(e) {
        if(window.location.hash === '' || window.location.hash === '#') ; //prevent regular page refreshes form effeting any change
        else {
                ...
                try{ //REVISIT THE DECISION TO UPDATE CERTAIN VISUAL ELEMENTS ONLY AFTER THE SCRIPTS ARE DONE DOWNLOADING (CONSIDER THE CASE WHEN CONNECTION IS VERY SLOW...)
                    switch(window.location.hash) {
                        ...
                        case('#list_albums'):
                            ...
                            if(westerb_sal.perms != 2) westerb_sal.exec_code_for_admin_list_albums();
                        ...
                        case('#list_photos'):
                            if(westerb_sal.get_url_param_value('to') === 'list_photos') { //if the page is being dispalyed after a login that had the 'to' URL parameter set, it will be redirected to 'list_albums.php'; execute the appropriate scripts for the page
                                ...
                                $('#main').fadeTo(50, 1, function() {
                                    setTimeout(function() {
                                        $('div.message_wrapper').slideUp(300, function() { $(this).remove(); }); //hide the Notice message that will be sent along
                                    },3000);
                                    westerb_sal.request_done = true;
                                });
                            } else {
...
'change_hash', { // Define Object.prototype.westerb_sal.change_hash
    value: function(store) {
        ...
        if(changer.attr('id') === 'home') {
            ...
            if(store) westerb_sal.store_main_page_state('#login', requested_pg_content);
            window.location.hash = 'login';
        }
        
        else if(changer.attr('id') === 'login') {
            if(store) westerb_sal.store_main_page_state('#'+(westerb_sal.get_url_param_value('to') || 'admin_index'), requested_pg_content);
            window.location.hash = westerb_sal.get_url_param_value('to') || 'admin_index';
        }
        
        else if(changer.attr('id') === 'gopublic') {
            if(store) westerb_sal.store_main_page_state('#public_index', requested_pg_content);
            window.location.hash = 'public_index';
        }
        
        else if(changer.attr('id') === 'gostaff') {
            if(store) westerb_sal.store_main_page_state('#admin_index', requested_pg_content);
            ...
            if(store) westerb_sal.store_main_page_state('#list_albums', requested_pg_content);
            ...
            if(store) westerb_sal.store_main_page_state('#photo_upload', requested_pg_content);
            window.location.hash = 'photo_upload';
        }

        else if(changer.text() === 'Create a new album') {
            if(store) westerb_sal.store_main_page_state('#create_album', requested_pg_content);
            window.location.hash = 'create_album';
        }
        
        else if(changer.text() === 'Add a new user') {
            if(store) westerb_sal.store_main_page_state('#add_user', requested_pg_content);
            window.location.hash = 'add_user';
        }

        else if(changer.text() === 'List users') {
            if(store) westerb_sal.store_main_page_state('#list_users', requested_pg_content);
            window.location.hash = 'list_users';
        }
        
        else if(/to_list_photos_/.test(changer.attr('id'))) {
            if(store) westerb_sal.store_main_page_state('#list_photos_'+changer.parent('div.album').attr('a_id'), requested_pg_content);
            ...
            if(store) westerb_sal.store_main_page_state('#logfile', requested_pg_content);
...
'get_hash_changer_requested_content', { // Define Object.prototype.westerb_sal.get_hash_changer_requested_content
    value: function(e) {
        if(!westerb_sal.request_done) westerb_sal.please_wait_msg(e);
        else if(westerb_sal.blocking_dialog) westerb_sal.scroll_to_dialog();
        //REVISIT THE else if LOGIC BELOW
                ...
                $(this).attr('id') !== 'login' && !westerb_sal.is_logged_in) westerb_sal.prompt_login(); //if the request is for going to any other page than the 'login' or 'public_index' pages, or to perform the actual login or logout, and the user is somehow logged out, prompt them to login first before continuing
        else {
            ...
            else if($(this).attr('id') === 'gologin') { url = westerb_sal.HOME+'admin/login.php'; requested_pg_name = 'login'; }

            else if($(this).attr('id') === 'login') {
                var after_login_pg = westerb_sal.get_url_param_value('to') || 'admin_index';
                url = westerb_sal.HOME+'admin/'+(after_login_pg==='admin_index'?'index':after_login_pg)+'.php'; requested_pg_name = after_login_pg;
            ...
            else if($(this).attr('id') === 'gopublic') { url = westerb_sal.HOME; requested_pg_name = 'public_index'; }

            else if($(this).attr('id') === 'gostaff') { url = westerb_sal.HOME+'admin/'; requested_pg_name = 'admin_index'; }
            
            else if($(this).attr('id') === 'to_admin_home') { url = westerb_sal.HOME+'admin/'; requested_pg_name = 'admin_index'; }
            //
            //
            //
            //
            else if($(this).attr('id') === 'to_albums') { url = westerb_sal.HOME+'admin/list_albums.php'; requested_pg_name = 'list_albums'; }
            //
            //
            //
            //
            else if($(this).attr('id') === 'logout') { url = westerb_sal.HOME+'admin/logout.php'; requested_pg_name = 'logout'; }
            
            else if($(this).text() === 'Upload a new photo') { url = westerb_sal.HOME+'admin/photo_upload.php'; requested_pg_name = 'photo_upload'; }

            else if($(this).text() === 'Create a new album') { url = westerb_sal.HOME+'admin/create_album.php'; requested_pg_name = 'create_album' }

            else if($(this).text() === 'Add a new user') { url = westerb_sal.HOME+'admin/add_user.php'; requested_pg_name = 'add_user' }

            else if($(this).text() === 'List users') { url = westerb_sal.HOME+'admin/list_users.php'; requested_pg_name = 'list_users' }

            else if($(this).text() === 'List albums') { url = westerb_sal.HOME+'admin/list_albums.php'; requested_pg_name = 'list_albums' }
            
            else if(/to_list_photos_/.test($(this).attr('id'))) { url = westerb_sal.HOME+'admin/list_photos.php?a_id='+$(this).parent('div.album').attr('a_id'); requested_pg_name = 'list_photos' }
            
            else if($(this).text() === 'View log file') { url = westerb_sal.HOME+'admin/logfile.php'; requested_pg_name = 'logfile' }
            ...
            if(window.location.hash === '#'+requested_pg_name || ($(this).attr('id') === 'home' && westerb_sal.cur_pg === 'public_index' &&
                ...
                !westerb_sal.get_url_param_value('a_id') && !westerb_sal.get_url_param_value('p_id')))) ; //no need to 'refresh' the page if the request is for the page that's already on display (will especially help prevent unnecessary "for-home" hash changes)
            else {
    ...
    restore_prev_state : {
        value: function(e) {
            ...
            //if($(this).text() == 'BACK TO ALBUMS') {
                ...
                if(to_main_home) westerb_sal.clear_full_screen_for_small();
                else {
                    $('#outer_wrapper, #big_error_report').fadeOut(80, function() {
                ...
                if(photo_count !== 0) { //there is at least one 'photo' to animate
                    (function fade_in_photo() {
                        $(photo[i]).show(duration, function() {
                            ...
                            if(i < photo_count) fade_in_photo();
//                            if(i === photo_count)
//                                console.log('\n\n'+Date().slice(16,24)+':'+new Date().getMilliseconds()+': The last photo in the album set being animated...');
    ...
    display_photo : {
            ...
            setTimeout(function() {
                ...
                //console.log('\n\n'+Date().slice(16,24)+':'+new Date().getMilliseconds()+': After the '+stall_time+'ms stall, the centering div is being appended the markup: \n'+content);
                $('div.centering:last').append(content);
                if(westerb_sal.selected_from_album) $('#to_albums_nav').removeClass('fullscreen_closer').text('BACK TO ALBUM');
                else $('#to_albums_nav').addClass('fullscreen_closer').text('BACK TO ALBUMS');
                westerb_sal.apply_mini_full_screen_styles();
    ...
    handle_get_album_for_display_response : {
        value: function() {
            if(westerb_sal.xhr_get_album_for_display.readyState === 4) {
                ...
                } else westerb_sal.handle_get_album_for_display_error();
            }
        }, writable: true, enumerable: true, configurable: true
    },
        ...
        value: function(e) {
            ...
            if($(this).scrollTop() === $(this).prop('scrollHeight') - Math.ceil($(this).innerHeight())) {//console.log('at the bottom of scroll...');
                if(westerb_sal.request_done) {
                    if(westerb_sal.blocking_dialog) { westerb_sal.scroll_to_dialog(); westerb_sal.highlight_dialog(e); }
                    else {
                        ...
                        if(!s_pagination.next_set()) westerb_sal.request_done = true;
                        else {
...
//                        fadeIn(150).fadeOut(150).fadeIn();
    ...
    confirm_del : {
        value: function(e) {
            ...
            } else westerb_sal.scroll_to_dialog();
            return false;
        }, writable: true, enumerable: true, configurable: true
    },
    ...
    modal_dialog : {
        value: function(evt, target, url) {
            ...
            else westerb_sal.modal_caller_parent.get(0).processing_msg('Loading');
            ...
            function handle_get_url_content_response() {
                if(xhr_get_url_content.readyState === 4) {
                    ...
                    if(xhr_get_url_content.status === 200) {
                        ...
                        westerb_sal.form = new westerb_sal.Form;
                        box.parent('div').css({'top':((window.innerHeight-(parseInt(box.css('height'))))/2)+westerb_sal.getScrollOffsets().y,'left':((window.innerWidth-(parseInt(box.css('width'))))/2)+westerb_sal.getScrollOffsets().x}).hide().css('visibility','visible').fadeIn(200, function() { $('#wait_msg').fadeOut(10, function() { $(this).remove(); }); westerb_sal.request_done = true; });
                        ...
                        $('#close_dialog').click(function(e) {
                            if(!westerb_sal.request_done || westerb_sal.form.xhr_in_progress) westerb_sal.please_wait_msg(e, true);
                            else close_box();
                        })

                        $('#close_dialog').hover(function() { $(this).css('background-color','rgba(19, 17, 17, 0.6)') }, function() { $(this).css('background-color','rgba(19, 17, 17, 0.3)') });
    ...
    upload_photo_by_modal : {
        value: function(e) {
            if(westerb_sal.blocking_dialog) westerb_sal.scroll_to_dialog();
            else if(!westerb_sal.request_done) westerb_sal.please_wait_msg(e);
    ...
    exec_code_for_admin_list_albums : {
        value: function() {
            ...
            name_div.hover(function() { $(this).css("opacity","1") },
                    ...
                    if(!pre_stored_albums) {
                        ...
                        if(westerb_sal.opened_album_groups.opened_count == 1) {
                            albums_wrapper.animate({'width':'1050px','min-width':''}, 200, 'linear', function() {
                                ...
                                $('.clicked_o_name').siblings('div.albums').slideDown(800, function() { $('#wait_msg').fadeOut(10, function() { $(this).remove(); }); westerb_sal.request_done = true; });
                            });
                            $('div.message_wrapper').slideUp(100);
                        } else {
                            $(this).siblings('div.albums').hide().html(pre_stored_albums);
                            westerb_sal.exec_code_for_album_group();
                            $(this).siblings('div.albums').slideDown(800, function() { $('#wait_msg').fadeOut(10, function() { $(this).remove(); }); westerb_sal.request_done = true; });
                        }
                    }
                } else {
                    if(westerb_sal.blocking_dialog) westerb_sal.scroll_to_dialog();
                        ...
                        $(this).siblings("div.albums").slideUp(800, function() { $('#wait_msg').fadeOut(10, function() { $(this).remove(); }); westerb_sal.request_done = true; });
                    }
                }
            });
    ...
    display_albums_section_msg : {
        value: function(sect_id, msg, type, duration) {
            if(!msg) msg = 'Something went wrong while updating the name. Refresh the page and try again.';
            if(!type) type = 'error';
            if(!duration) duration = 4000;
            ...
            $('#sect_msg').hide().text(msg).fadeIn(200, function() { westerb_sal.msg_remove_timer = window.setTimeout(function() { $('#sect_msg').fadeOut(200, function() { $(this).remove(); }) }, duration); });
            if(type === 'error' || type === 'success') westerb_sal.request_done = true;
        }, writable: true, enumerable: true, configurable: true
    },
    ...
    construct_notes_box : {
        value: function(a_id, notes_len, see) { //if "see" is true, the call was from "See notes"-- not "Add notes" and the box is adjusted accordingly
            if(see == null) see = false;
            if(notes_len == null) notes_len = 0;
            var str = '';
                ...
                if(westerb_sal.xhr_update_notes.status === 200) {
                    ...
                    if(westerb_sal.clicked_input_parent.hasClass('deleting_notes')) {
                        $('.album[a_id='+westerb_sal.clicked_input_parent.parent('form').parent('div.a_notes_wrapper').parent('div.album').attr('a_id')+']').each(function() {
                            ...
                            $(this).children('div.a_notes_wrapper').slideUp(400, function() { $(this).remove(); }); //slide off the screen the notes box on all relevant albums in different opened sections
                        });
                        ...
                        window.setTimeout(function() { westerb_sal.display_msg('Notes succesfully deleted', 'success', 2500); },430);
                    } else {
                        $('.album[a_id='+westerb_sal.clicked_input_parent.parent('form').parent('div.a_notes_wrapper').parent('div.album').attr('a_id')+']').each(function() {
                        ...
                        window.setTimeout(function() { westerb_sal.display_msg('Notes updated successfully', 'success', 2500); },30);
                    }
                } else {
        ...
        value: function(e) {
            if(westerb_sal.blocking_dialog) { westerb_sal.scroll_to_dialog(); westerb_sal.highlight_dialog(e); }
                ...
                if(westerb_sal.perms == 1) {
                    var what = '';
                    if(westerb_sal.clicked_input_parent.hasClass('deleting_notes')) what = 'delete';
                    else what = $(this).val() == 'Update changes'?'edit':'add';
                    westerb_sal.display_msg('As a guest, you do not have sufficient privileges to '+what+' album notes', 'notice');
                } else {
                    if(westerb_sal.clicked_input_parent.hasClass('deleting_notes')) westerb_sal.clicked_input_parent.get(0).processing_msg('Deleting');
                    else $(this).val() == 'Update changes'?westerb_sal.clicked_input_parent.get(0).processing_msg('Updating'):westerb_sal.clicked_input_parent.get(0).processing_msg('Adding');
    ...
    close_notes_box : {
        value: function(e) {
            if(westerb_sal.blocking_dialog) { westerb_sal.scroll_to_dialog(); westerb_sal.highlight_dialog(e); }
            else $(this).parent('div.notes_edit').parent('form').parent('div.a_notes_wrapper').slideUp(400, function() { $(this).remove() });
            return false;
        }, writable: true, enumerable: true, configurable: true
    },
    ...
    toggle_cancel : {
        value: function(e) {
            if($(this).attr('sentry') == 0 && $.trim($(this).val()) != e.data.initial_val) {
                ...
                if(e.data.see) $(this).siblings('div.notes_edit').children('span.char_counter').before("<input type='submit' value='Update changes' class='update_notes'>");
                else $(this).siblings('div.notes_edit').children('span.char_counter').before("<input type='submit' value='Add album notes' class='update_notes'>");
        ...
        value: function() {
            if(westerb_sal.xhr_get_notes.readyState === 4) {
                ...
                if(westerb_sal.xhr_get_notes.status === 200) {
                    ...
                    clicked_see_notes.parent('div.album').children('div.a_notes_wrapper').slideDown(400, function() { $('#wait_msg').fadeOut(10, function() { $(this).remove(); }); westerb_sal.request_done = true; });
                ...
                if($(this).children('div.notes').text() === 'See notes') {
                    ...
                    if(!pre_stored_notes) {
                        ...
                        if(westerb_sal.request_done) {
                            ...
                            $(this).parent('div.album').children('div.a_notes_wrapper').slideDown(400, function() { westerb_sal.request_done = true; });
                        } else ;
                    }
                }
    ...
    visualize_del_album_on_list_albums : {
          value: function(album) {
                ...
                if(westerb_sal.opened_album_groups.is_opened('a')) {
                    ...
                    if(remaining_albums === 1) inner_a_wrapper.children('p').children('span:last').fadeOut(250, function() { $(this).remove() });
                    ////
    ...
    handle_del_album_on_list_albums_response : {
          value: function() {
                if(westerb_sal.xhr_del_album_on_list_albums.readyState === 4) {
                    ...
                    album = $('.deleted_album');
                    if(westerb_sal.xhr_del_album_on_list_albums.status === 200) westerb_sal.visualize_del_album_on_list_albums(album);
                    else westerb_sal.display_albums_section_msg(album.parents('div.albums_by').attr('id'), 'Something went wrong while deleting the album. Refresh the page and try again.');
                }
          }, writable: true, enumerable: true, configurable: true
      },
...
Object.defineProperties(Object.prototype.westerb_sal, {
    display_tiny_msg : {
        value: function (message, type, duration) {
            ...
            var len = message.length;
                ...
                if(!duration) duration = 1900;
                if(len > 26) msg.attr('colspan','3');
                else if(len > 15) msg.attr('colspan','2');
                else msg.attr('colspan','1');
                $(document).scrollTop(0);
                msg.removeClass('success notice info error').addClass(type).html(message).fadeIn(50, function() { westerb_sal.msg_remove_timer = window.setTimeout(function() { msg.fadeOut(400) }, duration) });
                westerb_sal.request_done = true;
                $('#wait_msg').fadeOut(10, function() { $(this).remove(); });
            } else
    ...
    close_comments : {
        value: function(e) {
            if(westerb_sal.blocking_dialog) westerb_sal.scroll_to_dialog();
            else if(!westerb_sal.request_done && $(this).hasClass('del_in_progress')) westerb_sal.please_wait_msg(e); //incase deletion of comments is in progress when closing attempt is made
            else {
        ...
        value: function(e) {
            if(westerb_sal.blocking_dialog) westerb_sal.scroll_to_dialog();
                ...
                if(westerb_sal.perms == 1)
                    ...
                    westerb_sal.clicked_input_parent = $(this).parent('div');
                    var p_message, val = '';
                    if($(this).children('div').hasClass('delete')) { p_message = 'Deleting'; val = '' }
                    else { p_message = 'Updating'; val = encodeURIComponent(westerb_sal.clicked_input_parent.parent('form').children('.mini_form_edit_field').val().replace(' ', '+', 'gm')) }
        ...
        value: function() {
            if(westerb_sal.xhr_del_photo.readyState === 4) {
                ...
                if(westerb_sal.xhr_del_photo.status === 200) {
                    clicked_parent.parent('tr').slideUp(400, function() { $(this).remove(); });
                    ...
                    if(after_del_u_count == 1) {
                        $('#plural_s').text('');
                        $('#del_all').parent('span').prev().fadeOut(300, function() { $(this).remove() });
                        $('#del_all').parent('span').fadeOut(300, function() { $(this).remove() });
                    }
                    if(after_del_u_count == 0) {
                        $('#plural_s').text('s');
                        $('#del_album').parent('span').prev().fadeOut(300, function() { $(this).remove() });
                        $('#del_album').parent('span').fadeOut(300, function() { $(this).remove() });
    ...
    restore_pre_simulation_state : {
        value: function(e) {  //used when the user is Guest to restore the correct photo count and ux_nav_guide numbers when the next/prev link is clicked
            if($(this).children('div').hasClass('inactive')) //if the current page is 1 ('previous link' would be inactive in this case) or there are no more sets to fetch ('next link' would be inactive in this case)
                $('tr.simulation_row').fadeOut(400, function() { $(this).remove(); }) //remove the simulation rows, if the link happens to be clicked
            westerb_sal.pagination.adjust_prev_and_next();
            ...
            if($('#u_count').attr('p_count_before_simulation_starts') == 0) {
                $('span.del_album').prev('span.link_separator').fadeOut(400, function() {
                    $(this).next('span.del_album').fadeOut(400, function() { $(this).remove(); });
                    $(this).remove();
                })
                $('span.del_all_photos').prev('span.link_separator').fadeOut(450, function() {
                    $(this).next('span.del_all_photos').fadeOut(450, function() { $(this).remove(); });
                    $(this).remove();
                })
                $('#ux_nav_guide').text('0 of 0');
                $('#plural_s').text('s');
            }
            else $('#ux_nav_guide').text(westerb_sal.pagination.ux_nav_guide());
        ...
        value: function() {
            if(westerb_sal.xhr_get_more_comments.readyState === 4) {
                ...
                if(westerb_sal.xhr_get_more_comments.status === 200) {
                    ...
                    elems.each(function() { if(!$(this).hasClass('bound')) $(this).click(westerb_sal.del_com).addClass('bound'); });
    ...
    visualize_comment_deletion : {
        value: function() {
            ...
            if(remaining_comments === 1) clicked_parent_parent.siblings('p').children('span:last').fadeOut(250, function() { $(this).remove() });
            ...
            if(remaining_comments === 0) {
                clicked_parent_parent.parent('div').parent('td').parent('tr').slideUp(360, function() {
                    ...
                    if(westerb_sal.s_pag_instances['p_id_'+clicked_parent_parent.parent('div').parent('td').parent('tr').prev('tr').attr('p_id')]) delete westerb_sal.s_pag_instances['p_id_'+clicked_parent_parent.parent('div').parent('td').parent('tr').prev('tr').attr('p_id')]; //house cleaning
                    $(this).remove();
                });
        ...
        value: function(d_trigger_parent, what) {
            ...
            function album_erase() {
                ...
                (function remove_photo() {
                    row = rows[h];
                    row.fadeOut(450, function() { row.remove() });
                    h--;
                    if(h >= 0) {
                        if(row.attr('p_id')) //only if the removed row was a photo row ( and not a comments row)
                            window.setTimeout(function() { u_count.text(parseInt(u_count.text(),10)-1); $('#plural_s').text(h === 1 ? '':'s'); }, 450);
                        ...
                        if(h === 0)
                            window.setTimeout(function() {
                                ...
                                if(what === 'photos') {
                                    ...
                                    del_album.fadeOut(300, function() { del_album.remove() });
                                    del_all.prev().remove();
                                    del_all.fadeOut(400, function() { del_all.remove() });
                                    ...
                                    window.setTimeout(function() { westerb_sal.clear_p_timers(); location = 'list_albums.php'; }, 2000);
                                }
                                westerb_sal.request_done = true;
                            },700);
                    }
    ...
    del_all_photos : {
            value: function(d_trigger_parent, what) {
            westerb_sal.request_done = false;
            if(westerb_sal.perms == 1) westerb_sal.visualize_all_photos_or_album_deletion(d_trigger_parent, what);
              else {
    ...
    visualize_all_comments_deletion : {
        value: function(d_trigger_parent) {
            ...
            (function remove_comment() {
                comment = comments[i];
                comment.slideUp(300, function() { comment.remove() });
                i--;
                if(i >= 0) {
                    window.setTimeout(function() { d_trigger_parent.parent('p').children('span:first').text(i+(i === 1 ? ' comment':' comments')); }, 300);
    ...
    del_all_comments : {
            value: function(d_trigger_parent) {
            westerb_sal.request_done = false;
            if(westerb_sal.perms == 1) westerb_sal.visualize_all_comments_deletion(d_trigger_parent);
              else {
                            ...
                            $(this).prev('td').slideUp(150, function() {
                                $(this).parent('tr').slideUp(100, function () {
                                    ...
                                    if(deleted_u_id === westerb_sal.cur_u_id) { //if the user is deleting herself
                                        ...
                                        window.setTimeout(function() { window.location = '..'; },4300);
                                    } else westerb_sal.display_msg('The user was successfully deleted', 'success', 4000);
                                });
                            });
                        });
                    });
        ...
        value: function() {
            if(westerb_sal.xhr_del_user.readyState === 4) {
                ...
                if(westerb_sal.xhr_del_user.status === 200) {
                    if(westerb_sal.perms != 1) westerb_sal.clear_main_page_state(); //clear the '#main' div state for the 'list_users.php' page only if the current user isn't Guest (Guest's actions have no consequences)
                                ...
                                $(this).parent('tr').slideUp(slideup_duration, function () {
                                    ...
                                    p_wrapper.width(function(i,cur_val) {
                                        if(parseInt(cur_val) +  linear_increment > 400)
                                            return 400;
                                        else return parseInt(cur_val) +  linear_increment;
                                    }); //
                                    ...
                                    if(parseInt(p_percent.text())+percent_increment > 100)
                                        p_percent.text('100%');
                                    else p_percent.text(parseInt(p_percent.text())+percent_increment+'%');
                                });
                            });
                        });
                    });
...
$(window).resize(function() {
    if($('#outer_wrapper').hasClass('selected_photo_wrapper')) westerb_sal.readjust_gallery_elems();
});
...
$(window).resize(function() {
    if(!$('#outer_wrapper').hasClass('selected_photo_wrapper')) westerb_sal.readjust_general_public_elems();
});
...
$('body').on('click', '.scriptable_gen_nav_link.prev_set, .scriptable_gen_nav_link.next_set', function() {
    if(westerb_sal.cur_pg === 'list_photos' && westerb_sal.perms == 1) westerb_sal.restore_pre_simulation_state();
});
...
$('body').on('click', '.save_a_name_edit', function(e) { //REVISIT: WHAT ARE THE IMPLICATIONS OF BINDING 'click' INSTEAD OF 'submit' here?????
    if(!westerb_sal.request_done) westerb_sal.please_wait_msg(e);
        ...
        if($.trim($(this).data('unedited_a_name')) === $.trim($(this).siblings('input.mini_form_edit_field').val())) {
            if(westerb_sal.perms == 2) westerb_sal.display_msg('You did  not make any changes. Nothing to  update', 'notice', 2500);
            else westerb_sal.display_albums_section_msg($(this).parents('div.albums_by').attr('id'), 'You did  not make any changes. Nothing to  update', 'notice', 2500);
        }
        else if(westerb_sal.perms == 1) westerb_sal.display_albums_section_msg($(this).parents('div.albums_by').attr('id'), 'As a guest, you do not have sufficient privileges to edit album names', 'notice', 3000);
        else {
            westerb_sal.request_done = false;
            ...
            $.post(westerb_sal.HOME+'ajax/misc_calls.php?enma_id='+cur_album.attr('a_id'), {a_name:$(this).siblings('input.mini_form_edit_field').val()}).success(function(responseText, $_status, xhr) {
                ...
                if(westerb_sal.perms == 2) westerb_sal.display_msg('The album name was successfully updated', 'success', 2500);
                else westerb_sal.display_albums_section_msg(sect_id, 'The album name was successfully updated', 'success', 2500);
            }).error(function(xhr, $_status) {
                westerb_sal.clear_p_timers();
                $('#temp_a_processing_msg').remove();
                if($_status === 'timeout') {
                    if(westerb_sal.perms == 2) westerb_sal.display_msg('The request is taking too long. Please refresh the page and try again', 'notice', 3000);
                    else westerb_sal.display_albums_section_msg(sect_id, 'The request is taking too long. Please refresh the page and try again', 'notice', 3000);
                    westerb_sal.request_done = true;
                }
                else if ($_status === 'error')
                    if(westerb_sal.perms == 2) westerb_sal.display_msg('Something went wrong while updating the name. Refresh the page and try again.');
                    else westerb_sal.display_albums_section_msg(sect_id);
            });
        }
    }
    return false;
